...
use nalgebra::{ArrayStorage, Const, Matrix, Rotation2, SimdComplexField, SimdRealField, SVector};
...
impl<const R: usize> From<Cartesian<f32, R, Pixel>> for Cartesian<f32, R, Meter> {
    fn from(value: Cartesian<f32, R, Pixel>) -> Self {
        ...
        Self {
            elems: value.elems.map(|x| scale * x),
            unit: Meter::unit()
        }
    // todo!()
    }
...
impl<F: SimdRealField, const R: usize, U: DistanceUnit> Quantity for Cartesian<F, R, U> { type UNIT = U; }

pub trait Quantity {
    type UNIT: Unit;
}

impl<U: DistanceUnit> Cartesian<f32, 2, U> where Cartesian<f32, 2, U>: Sub<Cartesian<f32, 2, U>, Output=Cartesian<f32, 2, U>> {
    ...
    pub fn rotate_perpendicular_to(self, other: &Cartesian<f32, 2, U>, angle: f32) -> SVector<f32, 2> {
...
impl<X, const R: usize, const C: usize> Quantity for Velocity<X, R, C> where
    X: Tensor<R, C> + Quantity, { type UNIT = X::UNIT; }
...
impl Velocity<Cartesian<f32, 2, Meter>, 2, 1> {
    ...
    pub fn from_acceleration(accel: &Acceleration<Cartesian<f32, 2, Meter>, 2, 1>, step: &f32) -> Self {
        Velocity {
            elems: Cartesian {
